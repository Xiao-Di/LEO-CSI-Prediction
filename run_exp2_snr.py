"""
实验二: 不同 SNR 下的 NMSE (对应论文 Fig.6)
训练策略: 全部 SNR 混合训练 → 所有 SNR 直接推理 (无 MAML)
数据划分: 每个 SNR 10000 样本 → train 8000 (80/20 训练/验证) + test 2000
...
MIXED_CSV = "./dataset/Exp2_Mixed_train.csv"
...
def run_exp2(snrs=None, data_dir="./dataset"):
    if snrs is None:
        snrs = [0, 5, 10, 15, 20, 25, 30]

    train_files = [f"{data_dir}/Exp2_Speed100_SNR_{s}_train.csv" for s in snrs]
    test_files = [f"{data_dir}/Exp2_Speed100_SNR_{s}_test.csv" for s in snrs]
    config_name = "SNR_dB"
    ...
    for m_name, m_class in model_classes.items():
        ...
        pd.DataFrame({"Epoch": range(1, stopped_epoch + 1), "Loss": loss_curve}).to_csv(
            f"Exp2_{m_name}_Loss.csv", index=False
        )
        torch.save(model.state_dict(), f"Exp2_{m_name}_pretrained.pth")
        ...
        for f_path in test_files:
            ...
            avg_nmse, details = evaluate_model(
                ...
                config_label=config_name, config_value=config_val,
            )

            final_results.append({"Model": m_name, config_name: config_val, "NMSE_dB": avg_nmse})
            all_details.append(details)

            print(f"| {m_name} | {config_name}={config_val} | NMSE: {avg_nmse:.5f} dB |")

    nmse_df = pd.DataFrame(final_results)
    details_df = pd.concat(all_details, ignore_index=True)

    nmse_df.to_csv("NMSE_Results_SNR_Comparison.csv", index=False)
    details_df.to_csv("Exp2_Real_vs_Pred.csv", index=False)
    print("\n实验二完成！")
